...
class Extension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        print("[INFO] Extension root:", self._ext_root)
        ...
        # Max value (normalization factor)
        self._max_val = None
        ...
        try:
            ...
            print("[INFO] Window menu item added: Window -> TSF-85 Tactile Sensor")
        except Exception as e:
    ...
    def _resolve_ext_root(self, ext_id) -> Path:
        ...
        try:
            ...
            print("[WARN] Could not resolve ext path from manager:", e)
    ...
    def _load_nodes_csv(self):
        csv_path = nodes_csv_path(self._ext_root)
    ...
    def _load_norm_params(self):
        """Load the scalar normalization factor from norm_params.json.

        Expected file format:
            {"the_max": <float>}
        The grid is divided by this value before being fed to the CNN.
        """
        mp = norm_params_path(self._ext_root)
        print("[INFO] Norm-params path:", mp)
        if not mp.exists():
            print("[WARN] norm_params.json not found. Normalization disabled.")
            self._max_val = None
            return
        try:
            with open(str(mp), "r") as fh:
                data = json.load(fh)
            if not isinstance(data, dict) or "the_max" not in data:
                print("[WARN] norm_params.json missing 'the_max' key. Normalization disabled.")
                self._max_val = None
                return
            self._max_val = float(data["the_max"])
            print("[INFO] Loaded max value:", self._max_val)
        except Exception as e:
            print("[WARN] Failed to load norm params:", e)
            self._max_val = None
    ...
    def _load_onnx_model(self):
        ...
        print("[INFO] ONNX model path:", mp)
        if not mp.exists():
            print("[ERROR] ONNX model missing:", mp)
            self._session = None
            return
        ...
        try:
            ...
            print("[INFO] ONNX providers in use:", active)
            print(f"[INFO] ONNX input name: '{self._infer_input_name}'")
            ...
            if "CUDAExecutionProvider" not in active:
                print("[WARN] CUDA provider not active. Inference will run on CPU.")
        except Exception as e:
    ...
    def _has_any_deformable_api(self, prim) -> bool:
        """True if the prim carries any deformable schema (old or new)."""
        try:
    ...
    def _find_simulation_mesh(self, container_path: str):
        """Find the child whose name contains 'simulation_mesh', under a prim
        that carries a deformable API. Mirrors Test_pushing.py.
        Falls back to the first UsdGeom.Mesh under the container."""
        stage = self._ctx.get_stage()
        root = stage.GetPrimAtPath(container_path)
        if not root or not root.IsValid():
            return None

        # Preferred: explicit 'simulation_mesh' child under a deformable prim.
        for prim in Usd.PrimRange(root):
            if self._has_any_deformable_api(prim):
                for child in prim.GetChildren():
                    if "simulation_mesh" in child.GetName().lower():
                        return child

        # Fallback: first mesh under the container.
        for prim in Usd.PrimRange(root):
    ...
    def _ensure_deformable_cached(self, deform_container_path: str):
        ...
        # Current points and velocities live on the standard mesh attributes.
        pos_attr = mesh_prim.GetAttribute("points")
        ...
        self._dbg_once(
            "deform_schema",
            f"[INFO] Deformable mesh: {self._cached_deform_mesh_path} | "
            f"rest attr: {self._rest_attr_name}",
        )
        return True
    ...
    # =========================================================
    # DZ COMPUTE  (ported verbatim from Test_pushing.py)
    # =========================================================
    @staticmethod
    ...
    @staticmethod
    def _scale_to_cube(pts, cx, cy, cz, sx, sy, sz):
    ...
    def _compute_dz_from_arrays(self, rest_pts, curr_pts, R_mat, T, ordered_ids, all_node_ids):
        """Verbatim port of compute_dz_from_arrays from Test_pushing.py."""
        ...
        black = self._scale_to_cube(rot_f, cx_w, cy_w, cz_w, sx, sy, sz)
        orange = self._scale_to_cube(curr_f, cx_w, cy_w, cz_w, sx, sy, sz)

        return orange[:, 2] - black[:, 2]

    def _dz_to_grid(self, dz_array):
    ...
    def _predict_from_grid(self, grid_25x16):
        """Mirror infer_tactile from Test_pushing.py: normalize, clip, run ONNX."""
        if self._session is None:
            self._dbg_once("no_model", "[DBG] Inference disabled: ONNX model not loaded.")
            return None
        ...
        try:
            ...
            self._dbg_once("infer_run_err", f"[WARN] ONNX inference failed: {e}")
            return None
        ...
        while getattr(self, "_running", False):
            try:
                if self._infer_queue:
                    ...
                    self._dbg_once("pred_ok", f"[DBG] Predictions running (pred0={float(pred[0]):.4f})")
    ...
    def _on_physics_step(self, dt: float):
        ...
        if not self._ensure_deformable_cached(self._active_sensor_root_path):
            self._dbg_once("cache_fail",
                           f"[DBG] Could not find a deformable simulation mesh under "
        ...
        if sim_pts is None or rest_pts is None:
            self._dbg_once("pts_none",
                           f"[DBG] sim/rest points missing on {self._cached_deform_mesh_path}")
            return

        n = len(sim_pts)
        if len(rest_pts) != n:
            self._dbg_once("len_mismatch",
                           f"[DBG] points/restPoints length mismatch: {n} vs {len(rest_pts)}")
            return
        ...
        if len(ordered_ids) != EXPECTED_SIZE:
            self._dbg_once("idx_mismatch",
                           f"[DBG] Node ids mismatch: {len(ordered_ids)} != {EXPECTED_SIZE}")
            return
        ...
        try:
            ...
            self._dbg_once("dz_err", f"[WARN] dz computation failed: {e}")
            return

        dz = np.asarray(dz, dtype=np.float32).reshape(-1)
        if dz.size != EXPECTED_SIZE:
            self._dbg_once("dz_size", f"[DBG] dz size {dz.size} != {EXPECTED_SIZE}")
            return
