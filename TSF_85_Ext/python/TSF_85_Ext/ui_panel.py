...
class CoRoPanel:
    ...
    def __init__(self, ext):
        ...
        # Kept as attributes for backward compatibility with extension.py
        # which references them via hasattr(). They are intentionally None
        # so the writers in extension.py skip cleanly.
        ...
        self.log_dz_check = None
        self.log_pred_check = None
        self.log_mesh_check = None

        # Subscription handles for checkbox callbacks (keep refs so they
        # don't get garbage-collected and silently stop firing).
        self._log_dz_sub = None
        self._log_pred_sub = None
        self._log_mesh_sub = None
    ...
    def _build(self):
        ...
        self.window = ui.Window("TSF-85 Tactile Sensor", width=550, height=460)
        with self.window.frame:
            # ScrollingFrame so content is reachable even when the window is
            # smaller than the inner VStack. Both scrollbars appear when
            # needed; nothing gets clipped off-screen.
                ...
                with ui.VStack(spacing=0.3):
                    ...
                    with ui.CollapsableFrame("Output settings", collapsed=False, style={"margin": 3, "padding": 1}):
                        with ui.VStack(spacing=2):
                            ...
                            ui.Label("Outputs to generate:")
                            with ui.HStack(spacing=8, height=22):
                                ui.Spacer(width=6)
                                self.log_dz_check = ui.CheckBox(width=18)
                                self.log_dz_check.model.set_value(ext._log_dz_enabled)
                                self._log_dz_sub = self.log_dz_check.model.subscribe_value_changed_fn(
                                    lambda m: ext._set_log_flag("dz", m.get_value_as_bool())
                                )
                                ui.Label("Deformation maps", width=140)

                                self.log_pred_check = ui.CheckBox(width=18)
                                self.log_pred_check.model.set_value(ext._log_pred_enabled)
                                self._log_pred_sub = self.log_pred_check.model.subscribe_value_changed_fn(
                                    lambda m: ext._set_log_flag("pred", m.get_value_as_bool())
                                )
                                ui.Label("Synthetic data", width=120)

                                self.log_mesh_check = ui.CheckBox(width=18)
                                self.log_mesh_check.model.set_value(ext._log_mesh_enabled)
                                self._log_mesh_sub = self.log_mesh_check.model.subscribe_value_changed_fn(
                                    lambda m: ext._set_log_flag("mesh", m.get_value_as_bool())
                                )
                                ui.Label("Simulation data")
                    ...
                    with ui.CollapsableFrame("Deformable body information", collapsed=True, style={"margin": 3, "padding": 1}):
                        with ui.VStack(spacing=2):
                            ...
                            self.def_dz_count = ui.Label("Analyzed: (none)")
